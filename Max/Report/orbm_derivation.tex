...
The correction essentially applies a plus or minus a weight, or nothing depending on the state of the two RBMs. This allows one RBM to in effect change what the other RBM `sees` by either adding weight to a given visible or removing weight. This is almost equivalent to appling a correction to the visible vector that each RBM `sees`.
