...
\begin{itemize}
  \item \todowording{Can this model encode data comprised of more than one cause as it's constituted causes? That is, can the model and new algorithm for inverting it, perform source separation.}
  \item Is the ORBMs two cause structure to rich to be tractible in practice?
\end{itemize}

\section{Results and Contribution}

This project provides a suite of evalautions, in the form of tests in increasing complexity to answer the above questions. The aim being to incrementally verify the new generative model and inference algorithm.
